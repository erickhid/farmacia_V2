...
Partial Class RPProyeccion
    ...
    Private Sub LlenaAnos()
        ...
        DDL_ano.Items.Clear()
        For y As Integer = firstyear To currentYear + 1
    ...
    Protected Sub btn_grabar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_grabar.Click
        ...
        db.Cn1 = cn1
        usuario = Session("usuario").ToString()
        tipoR = "1" 'DDL_tipoR.SelectedValue
        Dim tbpacA As DataTable = db.ReporteProyeccion(tipoR, fechaA, fechaM, ultimodia, usuario)
        Session("dspacA") = tbpacA
        GV_rproyeccion.DataSource = tbpacA
        GV_rproyeccion.DataBind()
        lbl_titulo.Text = titulo(tipoR)
    End Sub

    Protected Sub GV_rproyeccion_DataBound(ByVal sender As Object, ByVal e As EventArgs) Handles GV_rproyeccion.DataBound
        If GV_rproyeccion.Rows.Count > 0 Then
            Dim TotalRows As Integer = GV_rproyeccion.Rows.Count
            Dim TotalCol As Integer = GV_rproyeccion.Rows(0).Cells.Count
            Dim FixedCol As Integer = 5
            Dim ComputedCol As Integer = TotalCol - FixedCol
            GV_rproyeccion.FooterRow.Cells(FixedCol - 1).Text = "Total: "
            For i As Integer = FixedCol To TotalCol - 1
                Dim sum As Integer = 0
                For j As Integer = 0 To TotalRows - 1
                    sum += If(GV_rproyeccion.Rows(j).Cells(i).Text <> "&nbsp;", Integer.Parse(GV_rproyeccion.Rows(j).Cells(i).Text), 0)
                Next
                GV_rproyeccion.FooterRow.Cells(i).Text = FormatNumber(sum.ToString("0"), 0, TriState.UseDefault, TriState.UseDefault, TriState.True)
    ...
    Protected Sub IB_exportar_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles IB_exportar.Click
        Dim nombre As String = "Reporte_ProyeccionARV_" & nombremes(DDL_mes.SelectedValue) & DDL_ano.SelectedValue
        Response.Clear()
    ...
    Protected Sub GV_rproyeccion_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles GV_rproyeccion.PreRender
        For Each r As GridViewRow In GV_rproyeccion.Rows
