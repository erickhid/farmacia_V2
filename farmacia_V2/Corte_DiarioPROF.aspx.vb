...
Partial Class Corte_DiarioPROF
    ...
    Sub llenaMedPROF()
        Try
            db.Cn1 = cn1
            usuario = Session("usuario").ToString()
            Dim Existencia_PROF As DataTable = db.ObtienePROF_FF_Existencias_D(usuario)
            Session("datosExistenciaPROF") = Existencia_PROF
            GV_pnl_CorteARV.DataSource = Existencia_PROF
        ...
        Catch ex As Exception
            ...
            errores = (usuario & "|Existencia_PROF.Inventario()|" & ex.ToString() & "|") + ex.Message
    ...
    Protected Sub ibt_generar_Click(sender As Object, e As ImageClickEventArgs) Handles ibt_generar.Click
        ...
        db.Cn1 = cn1
        db.Graba_Corte_PROF_Existencia(usuario)
    ...
    Protected Sub IB_exportar_Click(sender As Object, e As ImageClickEventArgs) Handles IB_exportar.Click
        ...
        Response.Clear()
        Response.AddHeader("content-disposition", "attachment;filename=" & "Corte_PROF" & "_" & fechasistema & ".xls")
